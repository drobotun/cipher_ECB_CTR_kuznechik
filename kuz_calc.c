...
GOST_Kuz_Expand_Key(const uint8_t *key)
{
    ...
    uint8_t iter_1[BLCK_SIZE];
    uint8_t iter_2[BLCK_SIZE];
    uint8_t iter_3[BLCK_SIZE];
    uint8_t iter_4[BLCK_SIZE];
    memcpy(key_1, key + BLCK_SIZE, BLCK_SIZE);
    memcpy(key_2, key, BLCK_SIZE);
    GOST_Kuz_Get_C();
    memcpy(iter_key[0], key_1, BLCK_SIZE);
    memcpy(iter_key[1], key_2, BLCK_SIZE);
    memcpy(iter_1, key_1, BLCK_SIZE);
    memcpy(iter_2, key_2, BLCK_SIZE);
    for (i = 0; i < 4; i++)
    {
        ...
        memcpy(iter_key[2 * i + 2], iter_1, BLCK_SIZE);
        memcpy(iter_key[2 * i + 3], iter_2, BLCK_SIZE);
    }
}
